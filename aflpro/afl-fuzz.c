...
#define MAX_NODE_NUM 100          // the maximum nodes in state machine
#define MAX_FUZZ_TIME_NODE 3000    // fuzzing times for each node then change the state
#define MAX_STATE_PATH 20         // the max execution path of state, avoid dead loop
#include "config.h"
#include "types.h"
#include "debug.h"
#include "alloc-inl.h"
...
//#include "library/json-c/json.h"  //Lingyun: add jason-c library
#include <json.h>
...
static u32 N_stateful_specified=0;    /* 1:stateful protocol, 0:stateless */
...
u32 current_state_id=0;               /* current protocol state id        */
u32 fuzz_count=0;                     /* record the fuzzing count         */
u32 Kill_By_CpuMem_Cost = 0;          /* kill_by_(receive msg or CpuMem)  */

/* create a structure for storing protocol state graph                    */
typedef struct _Field{
    u8* type;
    u8* defaultValue;
    u32 constraint;
    u32 fuzzable;                     /* 1== true, 0==false               */
}Field;
...
typedef struct _Node{
    ...
    u32 init;
    u32 inNum;
    u32 inNode[MAX_NODE_NUM];
    u32 outNum;
    u32 outNode[MAX_NODE_NUM];
    u32 FieldNum;
    Field format[MAX_NODE_NUM];

}Node;

typedef struct _Graph{
    u32 NodeNum;
    Node vexs[MAX_NODE_NUM];
}Graph;

Graph prograph;                       /* create a global protocol graph   */
u32 state_path[MAX_NODE_NUM];
u32 num_of_state_path=0;
u32 pre_num_of_state_path=0;          /* pre value of num of state path   */
...
static void _set_tcp_cork(int socket_fd, int status){
    int enable =status;
    if(setsockopt(socket_fd, SOL_SOCKET, TCP_CORK, (void*)&enable, sizeof(enable))==-1)
        PFATAL("failed to set_tcp_cork\n");
}

static void _set_tcp_nodelay(int socket_fd){
    int optval = 1;
    if(setsockopt(socket_fd, IPPROTO_TCP, TCP_NODELAY, &optval, sizeof(optval))==-1)
        PFATAL("failede to _set_tcp_nodelay\n");
}

static void _set_tcp_sndbuf(int socket_fd, int bufsize){
    int sendbuff=bufsize;
    if(setsockopt(socket_fd, SOL_SOCKET, SO_SNDBUF, &sendbuff, sizeof(sendbuff))==-1)
        PFATAL("setsockopt set sendbuffer failed\n");
}

int _get_tcp_sndbuf(int socket_fd){
    int sendbuff=0;
    socklen_t optlen=sizeof(sendbuff);
    if(getsockopt(socket_fd, SOL_SOCKET, SO_SNDBUF, &sendbuff, &optlen)==-1){
        PFATAL("getsockopt get sendbuffer failed");
    }else{
        return sendbuff;
    }
}

static void _set_socket_nonblocking(int socket_fd){

    u32 o;
    o = fcntl(socket_fd, F_GETFL);
    if (o >= 0) {
      o = o & (~O_NONBLOCK);
      if (fcntl(socket_fd, F_SETFL, o) < 0) {
        PFATAL("failed to reset non-blocking flag on socket descriptor (TCP)");
      }
    }
}

static void _set_tcp_mms(int socket_fd, int size){
    ssize_t ssize_max= size;
    if(setsockopt(socket_fd, IPPROTO_TCP, TCP_MAXSEG, &ssize_max, sizeof(ssize_max))==-1){
        PFATAL("setsockopt set sszie_max failed");
    }
}

ssize_t _get_tcp_mms(int socket_fd){
    ssize_t ssize_max=0;
    socklen_t optlen=sizeof(ssize_max);
    if(getsockopt(socket_fd, IPPROTO_TCP, TCP_MAXSEG, &ssize_max, &optlen)==-1){
        PFATAL("getsockopt get sszie_max failed");
    }else{
        return ssize_max;
    }
}

//send message by socket, call create_socket before using send_message
int send_leader_message(u8 * leader_msg, int len){
  u32 received = 0;
  u8 tempRcvBuffer[4096];
  if (N_results != NULL) {
    if (N_results->ai_socktype == SOCK_STREAM && N_fd != -1) {

        /*use sendfile() to transfer the data if possible because it is efficient*/
        int ssize_max=0, size_to_send=0;
        ssize_max = 4096*5;  //ssize_max set by _set_tcp_mms();
        if(len  > ssize_max){
            size_to_send = ssize_max;
        }else{
            size_to_send = len;
        }
        // create a temporary buffer to hold all the data in the file
        u32 writed=0;
        if((writed = write(N_fd, leader_msg, size_to_send))!=size_to_send){
            if(writed == -1 && errno != EAGAIN && errno != EWOULDBLOCK) {
                PFATAL("failed to send leader msg (TCP case)"); //send failed
            }
        }
        //ACTF("leader writed: %s, writed: %d, total_len: %d", leader_msg, writed, size_to_send);

        /*receive data from target server, thow all the reveived data*/
        if((received=read(N_fd,tempRcvBuffer,4096))>=0){
            // ACTF("success to receive data from server \n");
            memset(tempRcvBuffer,0,4096);
        }else{
            // ACTF("failed to receive data from server(leader msg stage)\n");
        }
        //ACTF("response of leader msg: %d\n", received);
        ...
        if (N_rp->ai_family == AF_INET) {
           ...
           * something wrong.  */
          if (sendto(N_fd, leader_msg, len, 0,
    ...
    FATAL("Create a socket before sending messages");
  }
  return 0;
}


//send the final mutated message by socket
int send_mutated_message(void){
  if (N_results != NULL) {
    if (N_results->ai_socktype == SOCK_STREAM && N_fd != -1) {
         ...
         * target process */
        s32 fd = dup(out_fd);    //temp_fd is the file_id for out_file
        ...
        lseek(fd, 0, SEEK_SET);
        /*use sendfile() to transfer the data if possible because it is efficient*/
        int ssize_max=0, sended=0, size_to_send=0;
        ssize_max = 4096*5;  //ssize_max set by _set_tcp_mms();
        ...
        if((sended= sendfile(N_fd, fd, NULL, size_to_send))!=size_to_send){
            if(sended == -1) { // if sendfile didn't work, use read() and write()
                lseek(fd, 0, SEEK_SET); // reset to the beginning of the file
                // create a temporary buffer to hold all the data in the file
                u8 tempbuf[statbuf.st_size];
                u32 writed=0,kread=0;
                if((kread=read(fd,tempbuf,size_to_send))==size_to_send){
                    if((writed=write(N_fd,tempbuf,kread))!=kread){
                        return -1; //failed to send data
                    }
                }else{
                    PFATAL("faile to read date to tempbuf for write (TCP Case)");
                }
            }else{
                return -1; // failed to send data, send again
            }
        }

        //ACTF("mutate sended: %d, total_len: %d", sended, size_to_send);

        /*receive data from target server, if no data is received, then kill the child_pid*/
        u32 received = 0;
        u8 tempRcvBuffer[4096];
        if((received=read(N_fd,tempRcvBuffer,4096))>0){
            /*kill the child_pid to solve the non-stop problem of protocol system*/
            Kill_By_CpuMem_Cost=1;
            kill(child_pid, SIGKILL);
        }else{
            //ACTF("failed to receive data from server (final mutate msg state)\n");
        }
        //ACTF(" response of mutate testcase: %d\n", received);
        ...
        //TOTEST:  ACTF("received mutate response: %d\n", received);
...
/* change the protocol state fuzzing now */
void change_state(void){

  pre_num_of_state_path = num_of_state_path;

  // determin the state is needed to be changed or not according to the fuzz_count
  if(fuzz_count==MAX_FUZZ_TIME_NODE){
      fuzz_count=0;
      u32 NextNodeNum=prograph.vexs[current_state_id].outNum;
      if(NextNodeNum >= 1){
        //choose the next node randomly.
        u32 random_choose=(random() % NextNodeNum);  // Note: the random() may bring in undeterministic problem 
        current_state_id=prograph.vexs[current_state_id].outNode[random_choose];
        num_of_state_path++;
        state_path[num_of_state_path]= current_state_id;

      }else if(NextNodeNum==0 || num_of_state_path > MAX_STATE_PATH){
        // clear the execution path informtion
        for(u32 i=0;i<num_of_state_path;i++)
          state_path[i]=0;
        num_of_state_path=0;
        current_state_id=0;
      }
...
//get the node's default valid message
u8* get_node_default_msg(u32 id){
  u32 Num=prograph.vexs[id].FieldNum;
  //the size should be considered.
  u32 total_len = 0;
  for(u32 i=0;i<Num;i++){
    u8* temp=prograph.vexs[id].format[i].defaultValue;
    total_len=total_len+strlen(temp)+1;
  }

  u8* result=ck_alloc(total_len+2);
  if(result == NULL) FATAL("malloc failed");
  for(u32 i=0;i<Num;i++){
    u8* temp=prograph.vexs[id].format[i].defaultValue;
    if(i==0){
        strcat(result,temp);
    ...
    if (N_results->ai_socktype == SOCK_STREAM) {
      ...
      if (N_myaddr_valid == 0) {
        ...
        for (N_rp = N_results; N_rp != NULL; N_rp = N_rp->ai_next) {
          ...
          _set_tcp_nodelay(N_fd);
          _set_socket_nonblocking(N_fd);
         // _set_tcp_mms(N_fd, 4096*5);//set larger ssize_max =40960

          /*set the socket options to send all the data when socket is cloesed
           * durding the l_linger time, 30 = 0.3 second*/

          struct linger lin;
          lin.l_onoff = 1;
          lin.l_linger= 0; //0: close socket and drop all data in socket sendbuffer
                           //30: close socket and try to send left data in 0.3s
          if(setsockopt(N_fd, SOL_SOCKET, SO_LINGER,&lin,sizeof(lin))== -1){
              PFATAL("failed to set socket option - SO_LINGER (TCP case)");
          }
        ...
        close(N_fd);              // ensure the socket has been closed
        ...
        _set_tcp_nodelay(N_fd);
        _set_socket_nonblocking(N_fd);
       // _set_tcp_mms(N_fd, 4096*5);//set larger ssize_max =4096*5

        /*set the socket options to send all the data in sendbuf when closed during l_linger time*/
      ...
      // add model guided stateful protocol fuzzing
      if(G_option_specified && N_stateful_specified){

        //model guided for stateful fuzzing
        if(prograph.vexs[current_state_id].init != 1){
          //send the leader messages based on the state_path
          for(u32 i=0;i<num_of_state_path;i++){
            u32 Node_id =state_path[i];
            u8* default_msg=get_node_default_msg(Node_id);
            u32 len=strlen(default_msg);
            if(send_leader_message(default_msg,len)==-1)
               return -1; // send leader message failed
            ck_free(default_msg);
          }
        }
        //send the mutaion msg finally
        if(send_mutated_message()==-1)
            return -1;
        fuzz_count++;
        change_state();
...
/*
        u8* default_msg="username";
        u32 len=strlen(default_msg);
        if(send_leader_message(default_msg,len)==-1)
            FATAL("username"); // send leader message failed

        u8* default_msg2="password";
        u32 len2=strlen(default_msg2);
        if(send_leader_message(default_msg2,len2)==-1)
            FATAL("password"); // send leader message failed
*/
        //stateless fuzzing
        if(send_mutated_message()==-1)
      ...
      int close_result;
      if((close_result=close(N_fd))!=0) FATAL("client socket close failed: %d", close_result);
      ...
      if (N_myaddr_valid == 0) {
        ...
        close(N_fd); //ensure the client socket has been closed;
        N_fd = socket(N_rp->ai_family, N_rp->ai_socktype, N_rp->ai_protocol);
        if (N_fd == -1) {
          PFATAL("Subsequent attempt to create socket failed (UDP case)");
          return -1;
        }
        ...
        if (bind(N_fd, (struct sockaddr *) (&N_myaddr), N_myaddrlen) == -1) {
          PFATAL("Attempt to bind socket to source address & port failed (UDP case)");
        }
      }
...
unsigned long min_pid_memcost=4294967294, max_pid_memcost=0;
/*check the cpu cost for child_pid process*/
static unsigned long getMemCost(s32 child_pid){
    if(child_pid <=0) FATAL("child_pid process is stopped");

    pid_t pid=child_pid;
    glibtop_union data;

    glibtop_get_proc_mem(&data.proc_mem,pid);
    /* dispaly all the memory consumption info of process pid */
    /*
    ACTF("Proc_Mem     PID  %5d (0x%08lx): "
            "MemSize: %lu  VirtulMemSize:%lu ResidentMem:%lu ShareMem: %lu RSS:%lu RSS_rlim: %lu\n", (int) pid,
    ...
    */
    unsigned long memory_cost;
    memory_cost = (unsigned long)data.proc_mem.resident;

    if(memory_cost>max_pid_memcost) max_pid_memcost = memory_cost;
    if(memory_cost<min_pid_memcost) min_pid_memcost = memory_cost;
...
static u8 run_target(char** argv) {
  ...
  Kill_By_CpuMem_Cost=0;    // reset the local variable to 0
  ...
  /*check the child_pid's memory and cpu consumption frequently, if continue 3 times
   * the memory and cpu consumption is extremely low or equal to zeor, then kill the
   * child_pid process, and set the local var is killbyCpu                        */

  /*
  if(N_valid){

    int Limit_Times=5, Mem_Stable_Count=0;
    double Total_Query_Time=(double)(exec_tmout-10);
    unsigned long pre_cost=0,cur_cost=0;
    while(Total_Query_Time>0){

        if(child_pid<=0) FATAL("Child Process is stopped!");

        cur_cost = getMemCost(child_pid);
        ...
        pre_cost=cur_cost;
        usleep(1000);
        Total_Query_Time=Total_Query_Time -1;   //update Total_Query_Time

        if(cur_cost <= max_pid_memcost && Mem_Stable_Count==Limit_Times){
           if(child_pid>0){
               //ACTF("kill child_pid: %d, cur_mem_cost:%lu, max_pid_memcost:%lu", child_pid, cur_cost, max_pid_memcost);
               kill(child_pid, SIGKILL);
               Kill_By_CpuMem_Cost=1;
               Mem_Stable_Count=0;
               break;
  ...
  */
  ...
  if(child_timed_out){
    if(N_valid){
     //return FAULT_NONE;         // for non-stop server, hang is normal and ok.
      return FAULT_HANG;       // if getMemCost enable, then return FAULT_HANG
    }else{
      return FAULT_HANG;
    }
  }
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  /*TOTEST: data write_to_outfile*/
  //ACTF("3 read into outfd: %s\n",mem);
...
/*fucntion used to determine the msg to be sent is matching the format or not*/
int match_format(char* msg){

  return 1; // for testing

  //get the node infomration by current_state_id
  Node *tempNode= &prograph.vexs[current_state_id];
  char *InputMsg=msg;
  //math each field's requirement
  for(int i=0;i<tempNode->FieldNum;i++){
    char * field_type=tempNode->format[i].type;                   //msg has been analy

    if(strcmp(field_type,"static")==0){
        //static field
        char *static_value=tempNode->format[i].defaultValue;      //get the default value
        int static_len=strlen(static_value);                      //get the length of default value
        char msg_test[static_len];
        if(strlen(InputMsg)<static_len){
          return 0;
        }
        strncpy(msg_test,InputMsg,static_len);
        msg_test[static_len]='\0';
        if(strcmp(msg_test,static_value)!=0) {
          return 0;
        }else{
          InputMsg=InputMsg+static_len;                           //move the pointer to the next field
        }
    }

    if(strcmp(field_type,"delimiter")==0){
        //delimiter field " "
        char *delim_value=tempNode->format[i].defaultValue;       //get the default value
        int delim_len=strlen(delim_value);                        //get the length of default value
        char msg_test[delim_len];
        if(strlen(InputMsg)<delim_len){
          return 0;
        }
        strncpy(msg_test,InputMsg,delim_len);
        msg_test[delim_len]='\0';                                 //get the front-length string of inputmsg
        if(strcmp(msg_test,delim_value)!=0) {                     //compare if the front-length string is equal to default value
          return 0;                                               //no, return;
        }else{
           InputMsg=InputMsg+delim_len;
        }
    }
    ...
    if(strcmp(field_type,"string")==0){
        //string field
        int str_constraint=tempNode->format[i].constraint;
        if(str_constraint==0) FATAL("constraint for string can not be 0");
        if(strlen(InputMsg)<str_constraint){
           return 0;
        }else {
           InputMsg=InputMsg+str_constraint;
        }
    }

    //must can be converted to int type
    if(strcmp(field_type,"int")==0){
        //string field
        int int_constraint=tempNode->format[i].constraint;
        char msg_test[int_constraint];
        if(strlen(InputMsg)<int_constraint){
          return 0;
        }
        strncpy(msg_test,InputMsg,int_constraint);
        msg_test[int_constraint]='\0';

        if(atoi(msg_test)==0){
          return 0;          //msg_test is not the type of integer
        }else{
          InputMsg=InputMsg+int_constraint;
        }
    }
...
/*added by Lingyu Situ*/
static u8 write_to_testcase_with_grammar(u8* mem, u32 len) {
  //check the grammar-format
  int result=match_format(mem);               // call mathch_format to check the grammar format

  if(result==0){                              // if the format is not matched, return 0;
    return 0;
  }
  ...
  if(N_valid){
    ...
    // other types of grammar base fuzzing
    write_to_testcase(mem, len);
    return 1;
  }
}
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if(G_option_specified){
      ...
      u8 res=write_to_testcase_with_grammar(use_mem, q->len);
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    /*TOTEST: data read from queue*/
    //ACTF("1 read from queue: %s\n",use_mem);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  //TOTEST:data after mutate
  // ACTF("2.2 data after mutate: %s\n", out_buf);
  u8 fault;
  ...
  if(G_option_specified){
      ...
      u8 res=write_to_testcase_with_grammar(out_buf, len);
    ...
    while ((qd_ent = readdir(qd))) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        if(G_option_specified){
          ...
          u8 res=write_to_testcase_with_grammar(mem, st.st_size);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -G dir        - use grammar-path to load grammar and model(Grammar mode)\n" 
...
/* create a protocol graph from input json file. */
Graph create_protocol_graph(u8* json_file){
  //create an instance of Graph
  Graph pgraph;  

  //load json object from local file
  struct json_object *protocol;
  protocol=json_object_from_file(json_file);

  enum json_type type;  
  json_object_object_foreach(protocol, key, val){
    //check the type of json object  
      ...
      switch(type){
        ...
        case json_type_boolean: 
            ACTF("json_type_booleann");
            ACTF("value: %d\n", json_object_get_boolean(protocol));
            break;
        case json_type_double: 
            ACTF("json_type_doublen");
            ACTF("value: %f\n", json_object_get_double(protocol));
            break;
        case json_type_int: 
            ACTF("json_type_intn");
            ACTF("value: %d\n", json_object_get_int(protocol));
            break;
        case json_type_string: 
            ACTF("json_type_string");
            ACTF("value: %s\n", json_object_get_string(protocol));
            break;
        case json_type_object:
            ACTF("json_type_object, the protocol descripiton is error!");
            break;
        case json_type_array: 
            ACTF("json_type_array");

            protocol= json_object_object_get(protocol, key);
            
            pgraph.NodeNum=json_object_array_length(protocol);
            ACTF("array length: %d",pgraph.NodeNum);


            for(int i=0;i<json_object_array_length(protocol);i++){
              struct json_object * MsgBlock;
              MsgBlock=json_object_array_get_idx(protocol, i);
              ...
              id=json_object_object_get(MsgBlock,"id");
              if(id ==NULL)
                FATAL("[id] is not exit, please check the descripiton!");
              pnode.id=json_object_get_int(id);

              //init
              struct json_object *init;
              init=json_object_object_get(MsgBlock,"init");
              if(init ==NULL)
                FATAL("[init] is not exist, please check the descripiton!");
              pnode.init=json_object_get_int(init);


              //inNode
              struct json_object *in;
              in=json_object_object_get(MsgBlock,"in");
              if(in ==NULL)
                FATAL("[in] is not exist, please check the descripiton!");
              pnode.inNum=json_object_array_length(in);
              for(int j=0;j<pnode.inNum;j++){
                pnode.inNode[j]=json_object_get_int(json_object_array_get_idx(in, j));
              }

              //outNode
              struct json_object *out;
              out=json_object_object_get(MsgBlock,"out");
              if(out ==NULL)
                FATAL("[out] is not exist, please check the descripiton!");
              pnode.outNum=json_object_array_length(out);
              for(int k=0;k<pnode.outNum;k++){
                pnode.outNode[k]=json_object_get_int(json_object_array_get_idx(out, k));
              }

              //format
              struct json_object *format;
              format=json_object_object_get(MsgBlock,"format");
              if(format ==NULL)
                FATAL("[format] is not exist, please check the descripiton!");

              int formatFiledNum=json_object_array_length(format);
              pnode.FieldNum=formatFiledNum;

              for(int m=0;m<pnode.FieldNum;m++){

                //each field
                struct json_object *BlockField;
                BlockField=json_object_array_get_idx(format, m);
                ...
                type=json_object_object_get(BlockField,"type");
                if(type ==NULL)
                  FATAL("[type] is not exit, please check the descripiton!");
                pfield.type=(u8*)json_object_get_string(type);

                //defaultValue
                struct json_object *defaultValue;
                defaultValue=json_object_object_get(BlockField,"defaultValue");
                if(defaultValue ==NULL)
                  FATAL("[defaultValue] is not exit, please check the descripiton!");
                pfield.defaultValue=(u8*)json_object_get_string(defaultValue);

                //constraint condition: length of string
                struct json_object *constraint;
                constraint=json_object_object_get(BlockField,"constraint");
                if(constraint ==NULL)
                  FATAL("[constraint] is not exit, please check the descripiton!");
                pfield.constraint=json_object_get_int(constraint);

                //fuzzable
                struct json_object *fuzzable;
                fuzzable=json_object_object_get(BlockField,"fuzzable");
                if(fuzzable ==NULL)
                  FATAL("[fuzzable] is not exit, please check the descripiton!");
                pfield.fuzzable=json_object_get_int(fuzzable);

                //add each field into pnode's format
                pnode.format[m]=pfield;

              }
              // add node into pgraph
              pgraph.vexs[i]=pnode;
            }

            ACTF("The protocol graph is created !");
            break;
    }
  }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:G:L:s")) > 0)
    ...
    switch (opt) {
      ...
      case 'N':
        ...
        if (N_slen > 0) {
          N_option_string = (u8*)ck_alloc(N_slen+1);  //N_option_string format: tcp:127.0.0.1:21
      ...
      case 's':  //stateful
        /*-s: represent the fuzzing mode is stateful */
        if(N_stateful_specified) FATAL("multiple -s options not allowed");
        N_stateful_specified=1;
        break;
  ...
  if (N_option_specified) {
    ...
    u8  *N_found1 = 0,         /*   :// ip:port               */
        *N_found2 = 0,         /*   :port                     */
        *N_pchar,              /* copy of TCP or UDP          */
        *N_type;               /* TCP or UDP                  */
    u8  *N_servicename = 0,    /* ptr to start of port number */
    ...
    if (N_fuzz_client) {
        ...
        if (!N_found2) {
          ...
          *N_found2 = 0;
          if (*(N_found2 + 1) == 0) {
            ...
            N_servicename = N_found2 + 1;       //record the port number
          }
        }
      }
      ...
      if ((strncmp(N_found1 + 1, "//", 2)) != 0) {
        ...
        *N_found1 = 0;
        N_hostspec = N_found1 + 3;             //record the ip address
      }
      if (!(
              (strcmp("localhost", N_hostspec) == 0)
              || (strcmp("::1", N_hostspec) == 0)
